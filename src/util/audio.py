...
def get_win_length():
    ...
    if win_length < 0:
        assert hp.win_length_ms > 0
        win_length = int(hp.win_length_ms / 1000 * hp.sample_rate)
    return win_length
...
def _stft(y, pad_mode="constant"):
    ...
    return librosa.stft(y=y, n_fft=hp.n_fft, hop_length=get_hop_size(),
                        win_length=get_win_length(), window=hp.window,
                        pad_mode=pad_mode)
...
def _normalize(S):  
    return np.clip((S - hp.ref_db + hp.max_db) / hp.max_db, 0, 1)


def _denormalize(S):
    return (np.clip(S, 0, 1) * hp.max_db) + hp.ref_db - hp.max_db
